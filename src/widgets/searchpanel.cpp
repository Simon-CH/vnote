...
void SearchPanel::setupUI()
{
    ...
    m_searchScopeComboBox->addItem(tr("CurrentFolder"), static_cast<int>(SearchScope::CurrentFolder));
    m_searchScopeComboBox->addItem(tr("CurrentNotebook"), static_cast<int>(SearchScope::CurrentNotebook));
    m_searchScopeComboBox->addItem(tr("AllNotebooks"), static_cast<int>(SearchScope::AllNotebooks));
...
TitleBar *SearchPanel::setupTitleBar(const QString &p_title, QWidget *p_parent)
{
...

    {
        ...
        m_cancelBtn = titleBar->addActionButton(QStringLiteral("cancel.svg"), tr("Cancel"));
        connect(m_cancelBtn, &QToolButton::triggered,
...
void SearchPanel::startSearch()
{
    ...
    // On start.
    {
        ...
        emit VNoteX::getInst().clearLocationListRequested();
    }
...
Searcher *SearchPanel::getSearcher()
{
    if (!m_searcher) {
        ...
        connect(m_searcher, &Searcher::resultItemAdded,
                ...
                /*
                    emit VNoteX::getInst().addToLocationListRequested(p_item->m_location,
                                                                      p_item->m_text,
                                                                      QString());
                                                                      */
                });
        connect(m_searcher, &Searcher::resultItemsAdded,
                ...
                /*
                    for (const auto &item : p_items) {
                        emit VNoteX::getInst().addToLocationListRequested(item->m_location,
                                                                          item->m_text,
                                                                          QString());
                    }
                    */
